class Country < ActiveRecord::Base
  ...
  def posts
    ...
    Post.where("id IN (?,?,?,?,?,?,?,?,?)",
                ...
                self.states.joins("INNER JOIN counties ON states.id = counties.state_id INNER JOIN places ON places.political_id = counties.id AND places.political_type = 'County' INNER JOIN posts ON posts.place_id = places.id").select("posts.id"),
                ...
                self.states.joins("INNER JOIN counties on states.id = counties.state_id INNER JOIN localities ON counties.id = localities.administrative_area_id AND localities.administrative_area_type = 'County' INNER JOIN places ON places.political_id = localities.id AND places.political_type = 'Locality' INNER JOIN posts ON posts.place_id = places.id").select("posts.id"),
                ...
                self.states.joins("INNER JOIN counties on states.id = counties.state_id INNER JOIN localities ON counties.id = localities.administrative_area_id AND localities.administrative_area_type = 'County'  INNER JOIN sublocalities ON sublocalities.locality_id = localities.id INNER JOIN places ON places.political_id = sublocalities.id AND places.political_type = 'Sublocality' INNER JOIN posts ON posts.place_id = places.id").select("posts.id"),
